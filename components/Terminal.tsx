...
export default function Terminal() {
  ...
  useEffect(() => {
    setLines([getWelcomeBanner(initialLocaleRef.current)]);
  }, []);

  useEffect(() => {
    if (locale !== initialLocaleRef.current) {
      initialLocaleRef.current = locale;
      setLines((prev) => [...prev, `\x1b[90m[Language changed to ${locale}]\x1b[0m`]);
    }
  }, [locale]);
