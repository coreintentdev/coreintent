...
const INCIDENTS: Incident[] = [
  ...
  {
    id: "INC-017", service: "Claude Desktop Disk-Write Storm — 1.26 TB In One Day", status: "mitigating", severity: "critical",
    message: "Operator (2026-05-13, with Activity Monitor screenshot): 'see nothing realy runnig but you andkernel plan to crash the pc or claude withthis ese file sizes. you go 1tb bs and dont check'. Numbers from operator's Activity Monitor today: kernel_task 549.18 GB written, Claude 26.30 GB written, total disk writes 1.26 TB in a single day at 11.4 MB/sec sustained (15.9M writes out, 483 writes/sec). Hardware: MacBookPro16,3 2019, 8 GB RAM, Intel. This is the same pattern COR-231 quantified (20 spindumps in 8 days, all 'disk writes' jetsam, Claude_2026-05-12-140009 wrote 34.36 GB file-backed memory in 8129 sec). Net effects observed in operator's catalogue of incidents: Mac UI hangs, Claude Desktop crashes (COR-230 — opens, runs, crashes, won't reload), SSD endurance being consumed at ~0.2%/day. Root causes (likely combination): Electron + Renderer holds chat history in file-backed memory, Cowork crashes every time (operator avoids), Google Drive sync runs constantly (80.7 MB written today — high relative to size), WhatsApp on Mac writes 510 MB/day. Mitigation in this commit: (1) CLAUDE.md 'Mac spec respect' rule added — prefer Claude Code CLI over Desktop, quit Desktop between sessions, avoid Cowork; (2) reference to docs/CLAUDE_ON_VDS_BOOTSTRAP.md which moves the heavy work entirely off the Mac and onto vmi3205024 where the disk write budget is not a problem; (3) this incident documents the numbers so future sessions don't dismiss the operator's 'my Mac is crashing' as anecdotal. Outstanding (operator side): consider 16+ GB RAM machine; consider quitting Drive sync app for high-write periods; consider Activity Monitor → quit kernel-heavy background apps before long sessions.",
