...
const INCIDENTS: Incident[] = [
  ...
  {
    id: "INC-012", service: "AI Location Inference From Stale Repo Files + False-Positive Security Alarm", status: "mitigating", severity: "major",
    message: "Operator (2026-05-13, verbatim): 'why claude think i am in NZ pl;ase is full proof of no CONTEXT - report major incidnt'. Then: 'I own a house in nicargua and thisi s mny o house wehre i work form'. This session inferred 'operator in NZ' from CLAUDE.md line 6 ('Based in: New Zealand') and the handover JSON line 9 — both written by prior sessions, never verified. The NZ assumption then propagated into a SECURITY false-positive: the Dropbox 'sign-in from Managua, Nicaragua' notice was flagged as a possible takeover (was actually operator's legitimate access from a personal work location). Apple 'recovery from Old Bridge NJ' remains unverified — without a wrong NZ baseline distorting the read, NJ recovery still looks anomalous but operator should verify directly. Pattern is the same as INC-002 / INC-005 / INC-009: AI sessions treat declarative statements in CLAUDE.md and handover docs as live truth without asking. Mitigation in this commit: (1) CLAUDE.md line 6 redacted; (2) docs/HANDOVER_COWORK_20260510.json same; (3) DESKTOP-HANDOVER-MASTER-2026-04-14.md same; (4) docs/SECURITY_HARDENING_CHECKLIST.md §0 created codifying the rule 'no affirmative country in any AI-context file'; (5) THIS incident documents the false-positive security alarm so future sessions don't trust 'location anomaly' calls without asking operator first. NOT TOUCHED (operator branding decision): marketing/product copy in app/page.tsx, app/layout.tsx, app/pricing/page.tsx, app/twitter-image.tsx, app/opengraph-image.tsx, app/demo/layout.tsx still contain 'Built in NZ' / 'en-NZ' / 'geo.region: NZ' in user-facing brand text and SEO meta. Operator must decide whether to keep, redact, or rewrite. Going-forward rule: if operator location matters operationally, ASK once explicitly; never infer from a committed file.",
    autoUpdate: false, detectedAt: "2026-05-13T00:00:00Z", updatedAt: "2026-05-13T00:00:00Z",
  },
  {
