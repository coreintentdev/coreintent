...
const OUTSOURCE_MAP: Record<string, { to: string; cost: string; why: string }> = {
  ...
  file_recovery:     { to: "google_drive_versions", cost: "$0",                     why: "30-day version history"             },
...
export async function POST(req: NextRequest) {
  ...
  const VALID_SAVE_TYPES = ["link", "doc", "state", "command", "config", "signal", "agent"] as const satisfies SaveType[];
