...
export default function DemoPage() {
  ...
  const obFlashTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      if (obFlashTimeoutRef.current) clearTimeout(obFlashTimeoutRef.current);
      obFlashTimeoutRef.current = setTimeout(() => setObFlash({}), 500);
    }, 800);
    return () => {
      clearInterval(iv);
      if (obFlashTimeoutRef.current) clearTimeout(obFlashTimeoutRef.current);
