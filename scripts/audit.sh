...
# OG tags (check root layout or locale layout)
...
if grep -q 'lang=' app/layout.tsx 2>/dev/null || grep -q 'langTag' app/layout.tsx 2>/dev/null; then
  log_pass "HTML lang attribute set (dynamic i18n)"
else
