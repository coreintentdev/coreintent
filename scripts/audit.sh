...
LINK_COUNT=$(grep -rn "from \"next/link\"\|from \"@/i18n/navigation\"" app/ components/ i18n/ 2>/dev/null | wc -l)
if [ "$LINK_COUNT" -gt 0 ]; then
  log_pass "Locale-aware links used in $LINK_COUNT files"
else
  log_fail "No internal navigation links found"
fi
...
if grep -q 'lang=' "app/[locale]/layout.tsx" 2>/dev/null; then
  log_pass "HTML lang attribute set (via i18n locale layout)"
else
